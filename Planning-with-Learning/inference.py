...
from utils.traj_utils import (
    get_front_traj,
    get_interpolated_traj_with_horizon,
    densify_offset_traj,
    get_offset_traj,
    global_to_local,
    local_to_global,
    bresenham_line_index,
    bresenham_line_point,
)
...
def main():
    ...
    csv_data = np.loadtxt(
        map_path + "/" + map_name + "_raceline.csv", delimiter=";", skiprows=0
    )  # '_centerline.csv'
    ...
    if method == "pure_pursuit":
        ...
        controller = KMPCController(
            model=KinematicModel(config=model_config),
            waypoints=kmpc_waypoints,
            config=model_config,
        )
    ...
    obt_index = np.random.uniform(
        1, waypoints.x.shape[0] - 1, size=(num_obstacles,)
    ).astype(int)
    thetas = np.arctan2(
        waypoints.x[obt_index + 1] - waypoints.x[obt_index - 1],
        waypoints.y[obt_index + 1] - waypoints.y[obt_index - 1],
    )
    obt_pose = (
        np.array([waypoints.x[obt_index], waypoints.y[obt_index], thetas])
        .transpose()
        .reshape((-1, 3))
    )
    ...
    # create & init env
    env = F110Env(
        map=map_path + "/" + map_name + "_map",
        map_ext=(
            ".pgm"
            if map_name == "levine_2nd"
            or map_name == "skir"
            or map_name == "skir_blocked"
            else ".png"
        ),
        num_agents=1,
        obt_poses=obt_pose,
    ...
    (
        ...
        if rl_planner or method == "kinematic_mpc"
        else None
    )
    (
        env.add_render_callback(renderer.render_lookahead_point)
        if rl_planner and method == "pure_pursuit"
        else None
    )
    (
        env.add_render_callback(renderer.render_offset_traj)
        if rl_planner or method == "kinematic_mpc"
        else None
    )
    ...
    init_pos = np.array(
        [waypoints.x[init_index], waypoints.y[init_index], waypoints.θ[init_index]]
    ).reshape((1, -1))
    ...
    while not done:
        if method == "pure_pursuit" and rl_planner:
            ...
            downsampled_lidar_scan = downsample_lidar_scan(
                obs["scans"][0].flatten(), rl_env.num_lidar_scan
            ).flatten()
            lidar_data = get_lidar_data(
                downsampled_lidar_scan,
                obs["poses_x"],
                obs["poses_y"],
                obs["poses_theta"],
            )
            ...
            front_traj = get_front_traj(
                obs, waypoints, predict_time=rl_env.predict_time
            )  # [i, x, y, v, θ]
            horizon_traj = get_interpolated_traj_with_horizon(
                front_traj, rl_env.horizon
            )  # [x, y, v, θ]
            local_horizon_traj = global_to_local(obs, horizon_traj)  # [x, y, v, θ]

            # gather agent obs & infer the action
            network_obs = np.hstack(
                (
                    downsampled_lidar_scan,
            ...
            )  # add car pose as the first point
            dense_offset_traj = densify_offset_traj(offset_traj)  # [x, y, v, theta]
            lookahead_point_profile = get_lookahead_point(
                obs, dense_offset_traj, lookahead_dist=rl_env.lookahead_dist
            )
            steering, speed = controller.rl_control(
                obs, lookahead_point_profile, max_speed=rl_env.fixed_speed
            )
            ...
            for i in range(offset_traj_indices.shape[0] - 1):
                line_indices = bresenham_line_index(
                    offset_traj_indices[i, :], offset_traj_indices[i + 1, :]
                )
                all_indices.append(line_indices)
            all_indices = np.concatenate(all_indices).reshape(-1, 2)
            filtered_traj_indices = all_indices[
                (all_indices[:, 1] < max_rows)
                & (all_indices[:, 0] < max_cols)
                & (all_indices[:, 1] >= 0)
                & (all_indices[:, 0] >= 0)
            ]
            ...
            for i in range(offset_traj.shape[0] - 1):
                line_points = bresenham_line_point(
                    offset_traj[i, :], offset_traj[i + 1, :]
                )
            ...
            # extract waypoints in predicted time & interpolate the front traj to get a 10-point-traj
            front_traj = get_front_traj(
                obs, waypoints, predict_time=1.0
            )  # [i, x, y, v, θ], 10Hz * 10 horizon
            horizon_traj = get_interpolated_traj_with_horizon(
                front_traj, rl_env.horizon
            )  # [x, y, v, θ]
            ...
            if int(lap_time * 100) % control_period == 0:  # 50 ms
                ...
                (
                    ...
                    mpc_oy,
                    a,
                ) = controller.rl_control(veh_state, mpc_offset_traj)
                renderer.offset_traj = np.array([ref_path_x, ref_path_y]).T  # red
                renderer.horizon_traj = np.array([pred_x, pred_y]).T  # yellow
